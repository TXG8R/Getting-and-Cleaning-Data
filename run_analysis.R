...
pathWork <- "D:/JoelW.WEBER/Dropbox/Coursera/Johns Hopkins - Data Science Certificate Program/3 - Getting and Cleaning Data/Course Project/UCI HAR Dataset/"
...
data.actvityType <- read.table("./activity_labels.txt", header = FALSE)
...
colnames(data.actvityType) <- c("ActivityID", "ActivityType")
...
# Join the data.results and data.actvityType by Activity ID so that meaningful Activity names will be available in the final result set, thus make the data more readable to end users
data.results <- merge(data.results, data.actvityType, by = "ActivityID", all.x = TRUE)
...
tidyData.final <- merge(tidyData.final, data.actvityType, by = "ActivityID", all.x = TRUE)
